...
- `skfenceha.yml.j2`: Docker Compose service definition
...
The error-pages service is automatically deployed as part of the skfenceha stack. Files are deployed to:
- `/var/data/config/skfenceha-{env}/error-pages/`
...
1. **Cluster Information Issues**:
   ...
   - Check docker-socket-proxy is running: `docker service ps skfenceha-{env}_socket-proxy`
   - Verify socket-proxy is accessible: `curl http://tasks.socket-proxy:2375/info`
   - Check service logs for API errors: `docker service logs skfenceha-{env}_error-pages`
2. **Container/Node Information Issues**:
   ...
   - Check service logs: `docker service logs skfenceha-{env}_error-pages`
