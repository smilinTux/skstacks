"""Every skfenceha router on the websecure entrypoint must carry a tls
section Traefik recognises. An empty `tls:` renders as YAML null, which
Traefik reads as no TLS, so the router never matches HTTPS and returns 404
(same class of bug fixed for skfence in test_skfence_router_tls.py; ported
here since routers.yml.j2 is deployed unchanged to both the worker and
ACME-master roles)."""
import pathlib
...
ROUTERS = pathlib.Path(__file__).resolve().parents[1] / "ansible/core/skfenceha/src/config/skfenceha/routers.yml.j2"
