...
_BACKEND_MAP: dict[str, str] = {
    ...
    "sops-age":         "secrets.sops_age.backend.SopsAgeBackend",
    "openbao":          "secrets.openbao.backend.OpenBaoBackend",
    "hashicorp-vault":  "secrets.hashicorp_vault.backend.HashiCorpVaultBackend",
    "capauth":          "secrets.capauth.backend.CapAuthBackend",
}

# Factory default = the server-less root of trust. It depends on NOTHING
# (just an ansible-vault password file), so bootstrapping never has a
# catch-22 where reading a secret requires a secret server that isn't up yet.
_DEFAULT_BACKEND = "vault-file"

# Recommended default *server* backend once you run one (sovereign, OSS/MPL-2.0,
# wire-compatible with HashiCorp Vault). See BOOTSTRAP.md for the trust chain.
_DEFAULT_SERVER_BACKEND = "openbao"
