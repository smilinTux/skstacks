...
## skos and v2: OS Consumer and Deployment Engine

**skos** is the sovereign operating system layer. **v2 is the deployment engine
that skos consumes** to provision, deploy, and manage capabilities.

```
skos (the OS)
  └─► v2 (deployment engine)
        ├─► secrets/        SKSecretBackend — the reference adapter pattern
        ├─► cloud/          edge · routing · naming · deploy · infra · dweb
        ├─► comms/          chat · voice · transport · bus
        ├─► compute/        data · cache · object · files · models · automation · obs · backup
        ├─► core/           identity · defense · WAF · PKI · secrets
        ├─► platform/       Swarm · K8s · RKE2 · k3d
        └─► infra/tofu/     OpenTofu provisioning modules
```

**Ports / Adapters:** every `sk*` directory is a **port** (stable capability
interface). The `provider:` field in `app.yaml` names the recommended **adapter**
(underlying technology). Adapters are swappable; the port interface is not.

**The 4C layout** (`v2/<C>/<port>/`) mirrors the secret control-plane taxonomy:
...
**`secrets/`** is the canonical reference implementation of the ports/adapters
pattern and the shared seam between v2 and any secret backend skos selects.
See `secrets/interface.py` (the port) and the three adapters beneath it.
...
flowchart TD
    ...
    subgraph STL["Service Template Layer"]
        T1["cloud/skfence/docker-compose.yml.j2"]
        T2["core/sksec/docker-compose.yml.j2"]
        T3["core/sksso/docker-compose.yml.j2"]
        T4["compute/skbackup/docker-compose.yml.j2"]
        T5["platform/swarm/ansible/roles/swarm-ha/templates/keepalived.conf.j2"]
    end
...
- **[CONVENTIONS.md](./CONVENTIONS.md)** — naming rules, ports/adapters methodology,
  how to add a new port or adapter, path reference, skos positioning. Read before
  extending v2.
- **[docs/APP-DESCRIPTOR.md](./docs/APP-DESCRIPTOR.md)** — complete `app.yaml` schema
  reference, field table, relationship between the v2 descriptor and the skos
